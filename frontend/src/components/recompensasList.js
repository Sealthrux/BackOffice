...
export default function servicoList() {
  ...
  useEffect(() => {
    const url = "https://backend-mygreentrip.com/recompensas/list/";
    axios
...
<div className="col-2"></div>
  ...
  function LoadFillData() {
    ...
    .map((data, index) => {
      return (
        <tr key={index}>
          <th>{data.idrecompensas}</th>
          <td>{data.descricao}</td>
          <td>
            <Link className="btn btn-outline-info" to={"/recompensasEdit/" + data.idrecompensas}>
              Edit
            </Link>
          </td>
          <td>
            <button
              ...
              onClick={() => notificationOnDelete(data.idrecompensas)}
            >
              {" "}
              Delete{" "}
            </button>
  ...
  function SendDelete(idrecompensas) {
    // url do backend
    console.log("vou apagar" , idrecompensas);

    const baseUrl = "https://backend-incomum.herokuapp.com/recompensas/delete/" + idrecompensas;
    // network
    axios
      .delete(baseUrl,  {headers: authHeader( localStorage.getItem("utilizadores"))})
      .then((response) => {
  ...
  function LoadServico() {
    const url = "https://backend-incomum.herokuapp.com/recompensas/list";
    axios
      .get(url,  {headers: authHeader( localStorage.getItem("utilizadores"))})
      .then((res) => {
        if (res.data.success) {
